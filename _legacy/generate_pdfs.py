...
def insert_dash_before_first_quote(text: str) -> str:
    # Inserta un em dash antes de la primera comilla doble si la línea comienza con comilla (tras espacios)
    leading_ws_len = len(text) - len(text.lstrip())
    stripped = text.lstrip()
    if stripped.startswith('"'):
        idx = text.find('"', leading_ws_len)
        if idx != -1:
            # Evita duplicar si ya hay un dash justo antes
...
def write_chunk_to_pdf(
    ...
    user_font_italic_path: Optional[str] = None,
    user_font_bold_path: Optional[str] = None,
):
    ...
    # Registrar exclusivamente las fuentes provistas (regular + italic [+ bold opcional]). Sin fallbacks.
    if not user_font_path or not user_font_italic_path:
        raise SystemExit("Debes especificar --font-regular y --font-italic (rutas TTF)")
    if not os.path.exists(user_font_path):
        raise SystemExit(f"No existe la fuente regular: {user_font_path}")
    if not os.path.exists(user_font_italic_path):
        raise SystemExit(f"No existe la fuente italic: {user_font_italic_path}")
    try:
        pdf.add_font("UserFont", style="", fname=user_font_path, uni=True)
        pdf.add_font("UserFont", style="I", fname=user_font_italic_path, uni=True)
        if user_font_bold_path:
            if not os.path.exists(user_font_bold_path):
                raise SystemExit(f"No existe la fuente bold: {user_font_bold_path}")
            pdf.add_font("UserFont", style="B", fname=user_font_bold_path, uni=True)
    except Exception as e:
        raise SystemExit(f"Error registrando fuentes: {e}")
    ...
    for chap_num, path in chunk_items:
        ...
        for raw in lines:
            ...
            if is_title:
                # Usamos negritas si se registró fuente bold, de lo contrario regular más grande
                style = "B" if user_font_bold_path else ""
                pdf.set_font(family_name, style=style, size=16)
                y = pdf.get_y()
                ...
                if stripped == "":
                    ...
                    # Texto: sin HTML; cursiva para líneas de diálogo que comiencen con comillas
                    is_dialogue = stripped.startswith('"')
                    if is_dialogue:
                        # Inserta em dash antes de la primera comilla
                        text = insert_dash_before_first_quote(text)
                        pdf.set_font(family_name, style="I", size=12)
                    else:
                        pdf.set_font(family_name, style="", size=12)

                    # Sangría al inicio de cada línea no vacía
                    indent_mm = 5
                    x0 = pdf.get_x()
    ...
    if add_toc and chapter_entries:
        pdf.add_page()
        style = "B" if user_font_bold_path else ""
        pdf.set_font(family_name, style=style, size=18)
...
def main():
    ...
    parser.add_argument(
        "--font-regular",
        default=os.path.join("fonts", "times-new-roman.ttf"),
        help="Ruta a la fuente TTF regular para todo el documento",
    )
    parser.add_argument(
        "--font-italic",
        default=os.path.join("fonts", "times-new-roman-italic.ttf"),
        help="Ruta a la fuente TTF italic (diálogos)",
    )
    parser.add_argument(
        "--font-bold",
        default=os.path.join("fonts", "times-new-roman-bold.ttf"),
        help="Ruta a la fuente TTF bold para títulos (opcional)",
    )
    parser.add_argument(
        "--basename",
        default="novela",
    ...
    for group in groups:
        ...
        write_chunk_to_pdf(
            ...
            user_font_path=args.font_regular,
            user_font_italic_path=args.font_italic,
            user_font_bold_path=(args.font_bold if args.font_bold else None),
        )
